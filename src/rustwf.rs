    ...
    let m = Command::new("rustwf")
            ...
            .subcommand(
                ...
                Command::new("wfSelection")
                    ...
                    .arg(arg!(-s <S> "Selection coefficient s").required(false).default_value("0.0").value_parser(value_parser!(f64)))
                    .arg(arg!(-d --dominance <H> "Dominance coefficient h").required(false).default_value("0.0").value_parser(value_parser!(f64)))
            ).subcommand(
                Command::new("conf3rm")
                    ...
                    .arg(arg!(-s <S> "Selection coefficient s").required(false).default_value("0.0").value_parser(value_parser!(f64)))
                    .arg(arg!(-d <D> "Conformity coefficient D").required(false).default_value("0.0").value_parser(value_parser!(f64)))
            ).subcommand(
                Command::new("conf5rm")
                    ...
                    .arg(arg!(--dthree <DTHREE> "Conformity coefficient D(3)").required(false).default_value("0.0").value_parser(value_parser!(f64)))
                    .arg(arg!(--dfour <DFOUR> "Conformity coefficient D(4)").required(false).default_value("0.0").value_parser(value_parser!(f64)))
            ).arg(arg!(-o --output <FOLDER> "Folder for the simulation outputs").required(false).default_value("./").value_parser(value_parser!(PathBuf)))
            ...
            .arg(arg!(-f --freq <FREQ> "Initial frequency of allele/variant A in the population").required(false).default_value("0.5").value_parser(value_parser!(f64)))
            .arg(arg!(-s --seed <SEED> "Random seed").required(false).default_value("42").value_parser(value_parser!(u64)))
            .arg(arg!(-r --rep <REP> "Number of simulations to run").required(false).default_value("1").value_parser(value_parser!(usize)))
            .arg(arg!(-t --threads <THREADS> "Number of parallel threads to run").required(false).default_value("1").value_parser(value_parser!(usize))).get_matches();
