pub mod sim {
    ...
    type IterFunction = Box<dyn Fn(&mut Vec<f64>, &mut SmallRng) -> f64 + Sync + Send>;

    /// function to run a single simulation in a single thread
    pub fn run(
        iteration: &IterFunction,
        ...
        write_result(result, seed, output_folder)
            .expect("Unable to write the simulation with id {seed} to .csv!");
    }
    ...
    pub fn wf_selection(s: f64, h: f64) -> IterFunction {
        Box::new(move |population: &mut Vec<f64>, rng: &mut SmallRng| {
            let p: f64 = population.iter().sum::<f64>() / population.len() as f64;
            let p_next =
                p + p * (1. - p) * ((1. + s) * p + (1. + h * s) * (1. - 2. * p) - (1. - p))
                    / ((1. + s) * p * p + 2. * (1. + h * s) * p * (1. - p) + (1. - p) * (1. - p));
    ...
    pub fn fpm_conformity_3rm(s: f64, d: f64) -> IterFunction {
        Box::new(move |population: &mut Vec<f64>, rng: &mut SmallRng| {
            let p: f64 = population.iter().sum::<f64>() / population.len() as f64;
            let p_next = (1. + s) * (p + d * p * (1. - p) * (2. * p - 1.))
                / (1. + s * (p + d * p * (1. - p) * (2. * p - 1.)));
            binomial_sample(population, p_next, rng);
            p
        })
    ...
    pub fn fpm_conformity_5rm(s: f64, d3: f64, d4: f64) -> IterFunction {
        Box::new(move |population: &mut Vec<f64>, rng: &mut SmallRng| {
            let p: f64 = population.iter().sum::<f64>() / population.len() as f64;
            let p_next = (1. + s)
                * (p + (1. - p) * p * (2. * p - 1.) * (d4 - p * (1. - p) * (d4 - 2. * d3)))
                / (1.
                    + s * (p
                        + (1. - p) * p * (2. * p - 1.) * (d4 - p * (1. - p) * (d4 - 2. * d3))));
            binomial_sample(population, p_next, rng);
            p
        })
    }

    pub fn fds_linear(a: f64, b: f64) -> IterFunction {
        Box::new(move |population: &mut Vec<f64>, rng: &mut SmallRng| {
            let p: f64 = population.iter().sum::<f64>() / population.len() as f64;
            let p_next = p * (-1. - 2. * (b - 1.) * p - (1. + a - 2. * b) * p * p)
                / ((1. + 2. * b) * (-1. + p) * p + a * (-1. + 3. * p - 3. * p * p));
            binomial_sample(population, p_next, rng);
            p
        })
    }
